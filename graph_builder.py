...
def create_graphrag_knowledge_graph(
    pairwise_data,
    strain_mean_biomass,
    microbial_abundance,
    metabolite_info,
    subsystem_scores_per_microbe,
):
    ...
    for strain, biomass in strain_mean_biomass.items():
        if pd.isna(biomass):
            ...
            continue
        G.add_node(
            strain, type="microbe", abundance=microbial_abundance.get(strain, None)
        )
    ...
    for pair in pairwise_data:
        ...
        for met_id, (flux, *_) in pair["crossfed metabolites"].items():
            ...
            G.add_edge(
                ...
                type="produces",
                flux=flux,
                description=f"{s1} produces {met_name} with a flux of {abs(flux)}",
            )
            G.add_edge(
                met_name,
                s2,
                type="consumes",
                flux=flux,
                description=f"{s2} consumes {met_name} with a flux of {abs(flux)}",
            )
        ...
        for idx, row in subsystem_scores_per_microbe.iterrows():
            ...
            if score > 0.0010:
                G.add_edge(
                    ...
                    description=f"{pathway} is involved in {microbe} as a pathway with importance score of {score}",
                )
...
def convert_nx_to_neo4j(G, neo4j_uri, neo4j_user, neo4j_password):
    ...
    def create_relationship(
        tx, source_name, source_labels, target_name, target_labels, rel_type, rel_props
    ):
...
list_EXrxns = sorted(
    {met for pair in pairwise_data for met in pair["crossfed metabolites"]}
)
list_EXrxns = [rxn[3:] for rxn in list_EXrxns]
metabolite_info = vmh_client.get_met_info(list_EXrxns)

microbial_abundance, xml_models_df = get_abundance(
    ABUNDANCE_CSV, MODELS_CSV, STRAINS_CSV
)
...
strain_mean_biomass = {
    id_to_strain[int(k)]: v
    for k, v in strain_mean_biomass.items()
    if int(k) in id_to_strain
}

subsystem_scores_per_microbe = pd.read_csv(
    "/Users/mohsen/Documents/vs projects/community_graphRAG/data/subsystem_scores_per_microbe.csv"
)

G = create_graphrag_knowledge_graph(
    pairwise_data,
    strain_mean_biomass,
    microbial_abundance,
    metabolite_info,
